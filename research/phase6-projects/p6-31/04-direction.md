# Design direction: The + and − buttons for the number of tickets are fiddly for older passengers.
...
**INFERRED:** input: touch (implied by platform kiosk); mode: audit (interaction defect on existing UI); mode: refactor (fix follows the diagnosis); project_theme: light-first (repository: hex palette: 8 near-white, 1 near-black); project_surfaces: elevated (repository: weak signal: shadow 1, border 1); project_radius: small (repository: most common radius 5 (3×); others [8.0, 12.0]); project_spacing: 4 (repository: most used spacing values [6, 12, 8])
