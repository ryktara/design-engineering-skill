...
(function () {
  ...
  function warnIdle() {
    ...
    $('#idle-count').textContent = graceLeft;
  ...
  function tickGrace() {
    ...
    $('#idle-count').textContent = graceLeft;
    // Announce the last few seconds instead of every tick, so the live region is
    // useful rather than chatty.
    if (graceLeft <= 3 && graceLeft > 0) {
      $('#idle-status').textContent = 'Clearing in ' + graceLeft + '…';
    }
  }
