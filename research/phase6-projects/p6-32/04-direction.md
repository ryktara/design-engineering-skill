# Design direction: Add a Welsh language option to the ticket machine.

**KNOWN:** platform: kiosk (request: ticket machine); stack: html-css (project inspection); environment: public (project inspection (README)); project_navigation: hub-spoke (repository: hub-spoke: 24 matches in README.md, app.js, i18n.js (shell/layout file))
**INFERRED:** input: touch (implied by platform kiosk); mode: create (build/create request on an existing surface); project_theme: light-first (repository: hex palette: 8 near-white, 1 near-black); project_surfaces: elevated (repository: weak signal: shadow 1, border 1); project_radius: small (repository: most common radius 5 (3×); others [8.0, 12.0]); project_spacing: 4 (repository: most used spacing values [6, 12, 8])
...
## Guardrails (required concerns: structure, interaction, accessibility, environment, privacy; uncovered: accessibility)
**accessibility**
