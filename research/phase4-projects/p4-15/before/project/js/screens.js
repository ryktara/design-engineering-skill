...
// Utility bar: present on every screen, always at the bottom (reach zone) — accessibility toggle, help, start over.
function utilbar(t, icons, state, { startOver = true } = {}) {
  return `<nav class="utilbar" aria-label="Kiosk options">
    <button class="btn btn--util" type="button" data-action="a11y" aria-pressed="${state.a11y}">${icons.a11y}<span>${state.a11y ? t.a11yOff : t.a11yOn}</span></button>
...
function errorLine(session, icons) {
  return `<p class="field__error" role="alert" id="field-error">${session?.error ? `${icons.alert}<span>${esc(session.error)}</span>` : ''}</p>`;
}
...
const screens = {
  attract({ t, icons, state }) {
    return `
      ...
      ${utilbar(t, icons, state, { startOver: false })}`;
  },
    ...
    return `${topbar(t, icons, 0)}
      <section class="screen" aria-labelledby="h">
        ...
        <div class="screen__body">
          <div class="field">
            <span class="field__label" id="dob-label">${t.dobLabel}</span>
            <div class="field__value" role="textbox" aria-readonly="true" aria-labelledby="dob-label" aria-describedby="field-error">
              ${d ? `<span>${fmt}</span><span class="caret" aria-hidden="true"></span>` : `<span class="ph">${t.dobPh}</span>`}
            </div>
            ${errorLine(session, icons)}
          </div>
          <div class="keys keys--pad" aria-label="${t.dobLabel}">
            ${keys}
  ...
  'identify-name'({ t, icons, state, session }) {
    ...
    const kb = rows.map((r, i) => {
      ...
      if (i === 2) return `<button class="key key--action" type="button" data-action="key" data-field="name" data-key="clear">${t.clear}</button>${ks}<button class="key key--wide" type="button" data-action="key" data-field="name" data-key="del" aria-label="${t.delete}">${icons.delete}</button>`;
      return ks;
    }).join('');
    return `${topbar(t, icons, 0)}
      <section class="screen" aria-labelledby="h">
        ...
        <div class="screen__body">
          <div class="field">
            <span class="field__label" id="name-label">${t.nameLabel}</span>
            <div class="field__value" role="textbox" aria-readonly="true" aria-labelledby="name-label" aria-describedby="field-error">
              ${session.name ? `<span>${esc(session.name)}</span><span class="caret" aria-hidden="true"></span>` : `<span class="ph">${t.namePh}</span>`}
            </div>
            ${errorLine(session, icons)}
          </div>
          <div class="keys keys--qwerty" aria-label="${t.nameLabel}">
            ${kb}
            <span aria-hidden="true"></span><span aria-hidden="true"></span>
            <button class="key key--action key--x4" type="button" data-action="key" data-field="name" data-key="-">-</button>
