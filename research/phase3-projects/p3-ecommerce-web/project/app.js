...
(function () {
  ...
  if (coForm) {
    ...
    let removed = false;
    function totals() {
      const subtotal = removed ? 0 : cart.price * cart.qty;
      ...
      const n = removed ? 0 : cart.qty;
      if (meta) meta.textContent = n + (n === 1 ? ' item' : ' items') + ' · ' + (ship ? money(ship) + ' shipping' : 'free shipping') + ' · tax incl.';
      const countEl = $('#summary-count');
      if (countEl) countEl.textContent = '(' + n + (n === 1 ? ' item' : ' items') + ')';
      $$('#place-order, #sticky-pay .btn').forEach((b) => {
        b.setAttribute('aria-disabled', removed ? 'true' : 'false');
        b.classList.toggle('is-disabled', removed);
      });
    }
    coForm.addEventListener('change', (e) => { if (e.target.name === 'shipping') totals(); });
    totals();

    /* Removing a line: reversible, never instant-destructive.
       The row stays as a 'Removed … Undo' strip until the order is placed, focus moves to Undo,
       and the existing polite live region announces both directions. */
    const lineItem = $('#line-item');
    const removedRow = $('#removed-row');
    const removeBtn = $('#remove-item');
    const undoBtn = $('#undo-remove');
    const emptyNote = $('#cart-empty-note');
    if (lineItem && removedRow && removeBtn && undoBtn) {
      const name = $('#summary-name').textContent.trim();
      $('#removed-name').textContent = name;
      removeBtn.setAttribute('aria-label', 'Remove ' + name + ' from your order');
      undoBtn.setAttribute('aria-label', 'Undo removing ' + name);
      const setRemoved = (v) => {
        removed = v;
        lineItem.hidden = v;
        removedRow.hidden = !v;
        if (emptyNote) emptyNote.hidden = !v;
        totals();
        statusEl.textContent = v
          ? name + ' removed from your order. Undo is available next to the summary.'
          : name + ' put back in your order.';
      };
      removeBtn.addEventListener('click', () => { setRemoved(true); undoBtn.focus(); });
      undoBtn.addEventListener('click', () => { setRemoved(false); removeBtn.focus(); });
    }
    ...
    coForm.addEventListener('submit', (e) => {
      e.preventDefault();
      if (removed) {
        summary.innerHTML = '<strong>Your order is empty</strong><p>Put the item back with Undo, or continue shopping.</p>';
        summary.hidden = false;
        statusEl.textContent = 'Your order is empty. Nothing was placed.';
        summary.focus();
        return;
      }
