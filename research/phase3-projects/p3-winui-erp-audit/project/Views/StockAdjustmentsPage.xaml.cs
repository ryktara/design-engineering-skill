...
public sealed partial class StockAdjustmentsPage : Page
{
    ...
    /// <summary>
    /// The row actions button selects its row and opens the one MenuFlyout shared by every row, so a
    /// realised row costs a single button instead of three, and the commands still read their target
    /// from ViewModel.Selected exactly like the command bar and the context menu do.
    /// </summary>
    private void RowActions_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not Button button) return;
        if (button.Tag is Models.StockAdjustment item) ViewModel.Selected = item;
        RowActionsFlyout.ShowAt(button);
    }
