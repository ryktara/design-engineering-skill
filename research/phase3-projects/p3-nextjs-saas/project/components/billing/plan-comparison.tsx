...
export function PlanComparison({ plans, currentPlanId, currentCycle, seatsInUse }: Props) {
  ...
  return (
    <section aria-labelledby="plans-heading" className="space-y-4">
      ...
      {/* Plan cards reuse the Card primitive (same surface, radius, padding and shadow as the dashboard stat cards). */}
      <ul className="grid gap-4 md:grid-cols-3" aria-label="Available plans">
        {plans.map((plan, i) => {
          ...
          return (
            <li key={plan.id} aria-current={isCurrent ? "true" : undefined} className="flex">
              <Card className={cn("flex w-full flex-col", isCurrent && "border-primary ring-1 ring-primary")}>
                <CardHeader className="pb-4">
                  <div className="flex items-start justify-between gap-2">
                    <CardTitle>{plan.name}</CardTitle>
                    {isCurrent && (
                      <Badge variant="default" className="-my-1 shrink-0 gap-1">
                ...
                <CardContent className="flex-1 space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{plan.seatLimit === null ? "Unlimited seats" : `Up to ${plan.seatLimit} seats`}</p>
                  </div>
                ...
                <CardFooter className="flex-col items-stretch gap-2">
                  {/* Note above the button so every card's CTA sits on the same baseline */}
                  {exceedsSeats && (
                    <p id={`${plan.id}-limit`} className="text-xs text-warning">
