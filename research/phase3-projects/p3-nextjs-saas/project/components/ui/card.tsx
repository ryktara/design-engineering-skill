...
// `asChild` lets a title/description render as another element (e.g. <dt>/<dd> inside a definition list)
// while keeping the card typography; same Slot convention as Button.
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement> & { asChild?: boolean }>(
  ({ className, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "h3";
    return <Comp ref={ref} className={cn("text-base font-semibold leading-none tracking-tight", className)} {...props} />;
  }
);
