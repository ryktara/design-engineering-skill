...
// `complete` marks the last step as finished: the step bar reads "Step 3 of 3 · Complete" instead of
// disappearing, so the progress indicator itself states the outcome in words.
function topbar(t, icons, stepIndex, complete = false) {
  const step = stepIndex == null ? '' :
    `<div class="step${complete ? ' step--done' : ''}" aria-live="polite">${t.stepOf(stepIndex + 1, 3)}<strong>${complete ? `${t.stepNames[stepIndex]} · ${t.stepComplete}` : t.stepNames[stepIndex]}</strong></div>`;
