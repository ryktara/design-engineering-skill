...
// `reference` is the value for the selected comparison period; the delta is derived from it.
  ...
  return (
    <>
      <div className={"kpi-delta " + tone}>
        ...
        <span className="kpi-delta-ctx"> {compare.short}</span>
      </div>
      <div className="kpi-ref">
        {compare.label}: <span data-numeric>{ref.value}{ref.unit ? " " + ref.unit : ""}</span>
      </div>
    </>
  );
}

export default function KpiGrid({ items, compare }) {
  return (
    <dl className="kpis">
      {items.map((k, i) => {
        ...
        return (
          <div className={"kpi" + (i === 0 ? " kpi-lead" : "")} key={k.id}>
            ...
            <dd className="kpi-body">
              ...
              <Delta value={k.value} reference={k[compare.id]} unit={k.unit} compare={compare} />
            </dd>
          </div>
        );
      })}
