...
class AppTheme {
  static ThemeData light({bool highContrast = false}) =>
      _build(Brightness.light, StatusColors.light, highContrast: highContrast);
  static ThemeData dark({bool highContrast = false}) =>
      _build(Brightness.dark, StatusColors.dark, highContrast: highContrast);

  static ThemeData _build(Brightness brightness, StatusColors status, {bool highContrast = false}) {
    final isLight = brightness == Brightness.light;
    // High contrast: secondary text becomes primary text, hairline outlines
    // become the strong outline, borders go from 2 to 3 dp. Colours already
    // clear 7:1; this is about non-text contrast and thin strokes in glare.
    final base = _scheme(isLight);
    final scheme = highContrast
        ? base.copyWith(
            onSurfaceVariant: base.onSurface,
            outlineVariant: base.outline,
            outline: isLight ? const Color(0xFF1B1F24) : const Color(0xFFF1F3F6),
            surfaceContainerHighest: isLight ? const Color(0xFFD9DDE3) : const Color(0xFF3A3F45),
          )
        : base;
    final stroke = highContrast ? 3.0 : 2.0;
    final focusStroke = highContrast ? 4.0 : 3.0;
    ...
    return _theme(scheme, status, stroke: stroke, focusStroke: focusStroke);
  }

  static ColorScheme _scheme(bool isLight) {
    return isLight
        ? const ColorScheme(
  ...
  static ThemeData _theme(ColorScheme scheme, StatusColors status, {required double stroke, required double focusStroke}) {
    ...
    return ThemeData(
      ...
      outlinedButtonTheme: OutlinedButtonThemeData(
        style: OutlinedButton.styleFrom(
          minimumSize: const Size.fromHeight(FieldSizes.control),
          shape: shape,
          side: BorderSide(color: scheme.outline, width: stroke),
          textStyle: textTheme.labelLarge,
          foregroundColor: scheme.onSurface,
        ),
      ),
      ...
      inputDecorationTheme: InputDecorationTheme(
        ...
        border: border(scheme.outline, stroke),
        enabledBorder: border(scheme.outline, stroke),
        focusedBorder: border(scheme.primary, focusStroke),
        errorBorder: border(scheme.error, focusStroke),
        focusedErrorBorder: border(scheme.error, focusStroke),
      ...
      dividerTheme: DividerThemeData(color: scheme.outlineVariant, thickness: stroke - 1, space: stroke - 1),
      switchTheme: SwitchThemeData(
        // Track outline follows the stroke so the off state reads in glare.
        trackOutlineWidth: WidgetStatePropertyAll(stroke),
      ),
