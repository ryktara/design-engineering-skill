          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                // The row's value says what the queue is doing right now,
                // the same words as the banner and the sheet.
                _SettingTile(
                  icon: sync.syncing
                      ? Icons.cloud_upload
                      : sync.hasFailures
                  ...
                  title: 'Sync queue',
                  value: sync.syncing
                      ? 'Sending ${sync.runSent + 1} of ${sync.runTotal} · ${sync.sending?.label ?? ''}'
                      : sync.hasFailures
                          ? '${sync.failed.length} failed to send · synced ${OfflineBanner.relative(sync.lastSyncedAt)}'
                          : sync.pending.isEmpty
