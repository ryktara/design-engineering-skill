...
import 'package:field_inspect/widgets/status_badge.dart';
...
void main() {
  testWidgets('checklist rows meet tap target and label guidelines', (tester) async {
    ...
    // Each row carries its own check control: a labelled 88 x 64 dp target,
    // separate from the row target that opens Pass / Fail / Skip.
    final check = find.bySemanticsLabel(RegExp(r'^(Mark .* passed|.* passed\. Undo)$'));
    expect(check, findsWidgets);
    expect(tester.getSize(find.byType(StatusBadge).first), const Size(88, 64));
    handle.dispose();
  });
