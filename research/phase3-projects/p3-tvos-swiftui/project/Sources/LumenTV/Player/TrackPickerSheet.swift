    ...
    private func column(title: String, tracks: [MediaTrack], selected: MediaTrack, width: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: LumenSpace.s) {
            ...
            ForEach(tracks) { track in
                TrackRow(track: track, isSelected: track == selected, isFocused: focus.wrappedValue == .picker(track.id)) { model.select(track) }
...
/// Selected ≠ focused: selection is the checkmark (+ accent text at rest); focus is the light fill + ring.
/// On the light focused fill the accent (#E8B04B) would be 1.9:1, so a focused row uses the dark label colour and
/// the checkmark alone carries "selected" (it is a glyph, not a colour, so nothing is lost).
        ...
        Button(action: action) {
            HStack(spacing: LumenSpace.s) {
                Image(systemName: "checkmark")
                    ...
                    .foregroundStyle(isFocused ? LumenColor.canvas : LumenColor.accent)
                    .opacity(isSelected ? 1 : 0)
                    .frame(width: 28)
                Text(track.displayName)
                    .font(LumenFont.body)
                    .foregroundStyle(isFocused ? LumenColor.canvas : (isSelected ? LumenColor.accent : LumenColor.textPrimary))
                    .lineLimit(1)
                Spacer(minLength: 0)
            }
