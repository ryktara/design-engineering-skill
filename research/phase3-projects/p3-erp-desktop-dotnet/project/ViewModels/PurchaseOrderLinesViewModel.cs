...
public sealed class PurchaseOrderLinesViewModel : ObservableObject
{
    ...
    private ColumnDefaults _defaults = ColumnDefaults.Load();
    private int _hiddenColumnCount;
    private bool _simulateFailure;
    private DateTime? _lastSyncAt;
    private bool _lastSyncFailed;
    private bool _syncCompact;
    private readonly DispatcherTimer _syncAgeTimer;

    public PurchaseOrderLinesViewModel()
    {
        FailedPostings = new ObservableCollection<FailedPosting>();
        ...
        RetryPostingsCommand = new RelayCommand(_ => RetryPostings(), _ => FailedPostings.Count > 0);

        FailedPostings.CollectionChanged += (_, _) => RaiseSync();
        // The sync age is relative text ("4 min ago"); re-read it on a slow tick so it never goes stale on screen.
        _syncAgeTimer = new DispatcherTimer(DispatcherPriority.Background) { Interval = TimeSpan.FromSeconds(30) };
        _syncAgeTimer.Tick += (_, _) => { OnPropertyChanged(nameof(LastSyncText)); OnPropertyChanged(nameof(LastSyncTooltip)); };
        _syncAgeTimer.Start();
    }
    ...
    /// <summary>Per-user defaults (default warehouse for new lines, columns kept hidden). Setting them persists them.</summary>
    public ColumnDefaults Defaults
    {
        get => _defaults;
        set { if (Set(ref _defaults, value)) { _defaults.Save(); OnPropertyChanged(nameof(DefaultWarehouse)); } }
    }
    public string DefaultWarehouse => Warehouses.Contains(_defaults.DefaultWarehouse) ? _defaults.DefaultWarehouse : Warehouses[0];
    ...
    // ---- ledger sync state (status bar) ---------------------------------------
    // Two facts the status bar carries: when this order last reached the ledger, and which postings did not.

    /// <summary>Postings the last sync could not write. Empty is the normal case and hides the segment.</summary>
    public ObservableCollection<FailedPosting> FailedPostings { get; }

    public DateTime? LastSyncAt { get => _lastSyncAt; private set { if (Set(ref _lastSyncAt, value)) RaiseSync(); } }
    public bool LastSyncFailed { get => _lastSyncFailed; private set { if (Set(ref _lastSyncFailed, value)) RaiseSync(); } }

    /// <summary>Absolute clock time plus the age in words, so the value is both precise and glanceable.</summary>
    public string LastSyncText => LastSyncAt is not DateTime t
        ? "Never synced"
        // The age is parenthesised, not dot-joined: the middle dot is the bar's segment separator and must not appear inside a segment.
        // Narrow windows drop the clock rather than truncate it mid-value; the full timestamp stays in the tooltip.
        : SyncCompact
            ? (LastSyncFailed ? $"Sync failed {Ago(t)}" : $"Synced {Ago(t)}")
            : (LastSyncFailed ? $"Sync failed {t:HH:mm} ({Ago(t)})" : $"Synced {t:HH:mm} ({Ago(t)})");

    /// <summary>Set by the view at narrow widths: the sync segment drops the clock time and keeps the age.</summary>
    public bool SyncCompact { get => _syncCompact; set { if (Set(ref _syncCompact, value)) OnPropertyChanged(nameof(LastSyncText)); } }

    public string LastSyncTooltip => LastSyncAt is not DateTime t
        ? "This order has never been synced with the general ledger"
        : (LastSyncFailed ? "Last ledger sync attempt failed: " : "Last ledger sync: ") + t.ToString("dddd d MMMM yyyy, HH:mm:ss");

    public int FailedPostingCount => FailedPostings.Count;

    public string FailedPostingSummary => FailedPostingCount == 0
        ? ""
        : $"{FailedPostingCount} posting{(FailedPostingCount == 1 ? "" : "s")} failed — Retry";

    /// <summary>Every failed posting named with its reason; a bare count is not actionable.</summary>
    public string FailedPostingDetail => FailedPostingCount == 0
        ? ""
        : string.Join(Environment.NewLine, FailedPostings.Select(p => $"{p.Reference} — {p.Reason} ({p.AttemptedAt:HH:mm})"));
    ...
    private void RaiseSync()
    {
        ...
        OnPropertyChanged(nameof(FailedPostingCount));
        OnPropertyChanged(nameof(FailedPostingSummary));
        OnPropertyChanged(nameof(FailedPostingDetail));
        CommandManager.InvalidateRequerySuggested();
    }

    private void RetryPostings()
    {
        var n = FailedPostings.Count;
        FailedPostings.Clear();
        LastSyncFailed = false;
        LastSyncAt = DateTime.Now;
        StatusMessage = $"Retried {n} posting{(n == 1 ? "" : "s")} — all posted to the ledger";
    }
    ...
    public ICommand RetryPostingsCommand { get; }
    ...
    public async Task LoadAsync(int delayMs = 700, bool empty = false)
    {
        ...
        try
        {
            ...
            FailedPostings.Clear();
            if (!empty) foreach (var f in SampleData.FailedPostings()) FailedPostings.Add(f);
            LastSyncFailed = false;
            LastSyncAt = SampleData.LastSync();
        }
        catch (Exception ex)
        {
    ...
    {
        var line = new PurchaseOrderLine { LineNo = (Lines.Count == 0 ? 0 : Lines.Max(l => l.LineNo)) + 10, ItemCode = "", Description = "", Warehouse = DefaultWarehouse };
        line.Validate();
