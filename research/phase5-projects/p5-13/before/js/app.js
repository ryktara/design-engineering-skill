...
export function dobProblem(d, strings) {
  if (d.length !== 8) return strings.errDob;
  const m = +d.slice(0, 2), day = +d.slice(2, 4), y = +d.slice(4, 8);
  const now = new Date();
  if (m < 1 || m > 12 || y < 1900 || y > now.getFullYear()) return strings.errDobInvalid;
  const dim = new Date(y, m, 0).getDate();
  if (day < 1 || day > dim) return strings.errDobInvalid;
  if (new Date(y, m - 1, day) > now) return strings.errDobInvalid;
  return null;
}
...
// ---------- actions ----------
function applyLang(lang) { state.lang = lang; document.documentElement.lang = lang; render(); paintChrome(); }
...
const actions = {
  ...
  'dob-continue': () => {
    const problem = dobProblem(session.dob, t());
    if (problem) { session.error = problem; render(); return; }
    go('identify-name');
  },
  find: () => { if (session.name.trim().length < 2) { session.error = t().errName; render(); return; } lookup({ dob: session.dob, lastName: session.name.trim() }); },
  toggle: (el) => {
