...
final class StoreTests: XCTestCase {
    ...
    func testWeeklyCounts() {
        let store = Store()
        XCTAssertEqual(store.weeklyCompletedCount, 25)
        XCTAssertEqual(store.weeklyTotalCount, 35)
        XCTAssertEqual(store.weeklyCompletionRate, 25.0 / 35.0, accuracy: 0.001)
    }
