...
final class Store: ObservableObject {
    ...
    var weeklyCompletedCount: Int { weeklyCompletions.reduce(0) { $0 + $1.completed } }
    var weeklyTotalCount: Int { weeklyCompletions.reduce(0) { $0 + $1.total } }

    var weeklyCompletionRate: Double {
        weeklyTotalCount == 0 ? 0 : Double(weeklyCompletedCount) / Double(weeklyTotalCount)
    }
