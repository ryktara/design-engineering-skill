...
/**
 * Reschedule one appointment (drag / keyboard move on the day grid).
 * Only the moved record gets a new object; every other item keeps its reference so
 * keyed `{#each}` blocks leave their DOM untouched.
 */
export function moveAppointment(id: string, practitionerId: string, start: string) {
  appointments.update((list) => {
    const i = list.findIndex((a) => a.id === id);
    if (i === -1) return list;
    const a = list[i];
    if (a.practitionerId === practitionerId && a.start === start) return list;
    const next = list.slice();
    next[i] = { ...a, practitionerId, start };
    return next;
  });
}
